...
The evolution of wireless networks toward 6G demands cryptographic algorithms capable of delivering unprecedented throughput while maintaining strong security guarantees. Current high-performance authenticated encryption schemes frequently leverage platform-specific SIMD instructions, particularly AES-NI on x86 architectures {{AES-NI}}, to achieve impressive speeds exceeding 100 Gbps. Notable examples include AEGIS {{!I-D.irtf-cfrg-aegis-aead}}, SNOW-V {{SNOW-V}}, and Rocca-S {{ROCCA-S}}.

However, this optimization strategy creates a significant performance disparity across different hardware platforms. While these algorithms excel on x86 processors equipped with AES-NI, they often exhibit substantially degraded performance on ARM architectures that implement SIMD functionality through NEON instructions. This inconsistency poses a critical challenge for modern network deployments where ARM processors dominate mobile devices, edge computing nodes, and increasingly, data center environments.
...
This document presents HiAE (High-throughput Authenticated Encryption), an authenticated encryption algorithm explicitly designed to address these cross-platform performance challenges. HiAE achieves competitive performance on both x86 and ARM architectures through careful algorithmic design that balances the strengths and constraints of each platform, making it particularly well-suited for the heterogeneous computing environments that will characterize 6G networks and beyond.
